(function($){
    $.fn.ckSlide = function(opts){
        ...
        this.each(function(){
            ...
            // next
            $(this).find('.right-point').on('click', function(){
            ...
            // prev
            $(this).find('.left-point').on('click', function(){
                if(opts['isAnimate'] == true){
                    return;
                }
			...
			//点击切换相应序号的图片
            $(this).find('.banner-nav-wrapper a').each(function(cindex){
                $(this).on('click.wrapper', function(){
			...
			//自己添加——鼠标移入小圆点切换轮播图片
			$(this).find('.banner-nav-wrapper a').each(function(cindex){
                $(this).on('mouseover.wrapper', function(){
                    change.call(that, cindex, index);
                    index = cindex;
                });
            });
            ...
            $(this).on('mouseover', function(){
                ...
                $(this).find('.point').css({opacity:0.3});
            });
            ...
            $(this).on('mouseleave', function(){
                ...
                $(this).find('.point').css({opacity:0.1});
            });
            startAtuoPlay(opts.interval);
            // 自动滚动播放
            ...
            // 修正box  标记居中
            var box = $(this).find('.banner-nav-wrapper');
            box.css({
    ...
    function change(show, hide){
        ...
        if(opts.dir == 'x'){
            ...
            $(this).find('.banner li').eq(show).show().css({opacity:0}).stop().animate({opacity:0.9});
        }
       //切换对应标记的颜色
        $(this).find('.banner-nav-wrapper a').removeClass('current');
        $(this).find('.banner-nav-wrapper a').eq(show).addClass('current');
    }
    $.fn.ckSlide.opts = {
